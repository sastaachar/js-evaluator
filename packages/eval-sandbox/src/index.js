...
export class SandboxedEval {
  ...
  #emit(type, payload) {
    ...
    for (const handler of [...set]) {
      try {
        ...
        console.error(`[eval-sandbox] "${type}" listener threw:`, err);
      }
    }
  }
  ...
  init() {
    ...
    iframe.setAttribute("title", "eval-sandbox");
  ...
  async #onFetchRequest(msg) {
    ...
    try {
      ...
      console.error("[eval-sandbox] fetch policy threw, blocking request:", err);
      allow = false;
    }
